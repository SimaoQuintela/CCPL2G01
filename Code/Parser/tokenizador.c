...
void tokenizador(STACK *s, char* token){
  ...
  else if(verifica_carater(token[0]) == 1)
    push_CHAR(s, token[0]); 

  else if(strcmp(token, "-") == 0)   // sinal -
      subtrai(s);
  else{
    switch(token[0]){
...
* @param carater Carater que é passado como parâmetro e que vai ser comparado com a string string_de_carateres.
